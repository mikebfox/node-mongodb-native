...
describe('Sharding (Connection)', function() {
  ...
  it('Should connect to mongos proxies using connectiong string and options', {
    ...
    test: function(done) {
      ...
      const client = new MongoClient(url, { haInterval: 500, useNewUrlParser: true });
  ...
  it('Should exercise all options on mongos topology', {
    ...
    test: function(done) {
      ...
      const client = new MongoClient(url, { useNewUrlParser: true, haInterval: 500 });
  ...
  it('Should correctly modify the server reconnectTries for all sharded proxy instances', {
    ...
    test: function(done) {
      ...
      const client = new MongoClient(url, { useNewUrlParser: true, reconnectTries: 10 });
      client.connect(function(err) {
        expect(err).to.not.exist;
        expect(client)
